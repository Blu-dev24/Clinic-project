  ...
  return (
    <header className="bg-gradient-to-r from-pink-100 to-purple-100 sticky top-0 z-50">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        <a
          href="/"
          className="flex items-center focus:outline-none focus-visible:ring-2 focus-visible:ring-pink-600 rounded"
        >
          <h1 className="text-2xl font-bold text-pink-600">Glamour Clinic</h1>
        </a>

        <nav aria-label="primary investigation">
              ...
              <ul className="flex flex-col space-y-4">
                {navLinks.map((link, index) => (
                  <li key={index}>
                    <a
                      href={link.href}
                      className="block text-gray-700 hover:text-pink-600 trasition-colors duration-200 py-2 px-2 focus:outline-none focus:ring-2 focus: ring-pink-600 focus:rounded"
                      onClick={toggleMenu}
                    >
                      {link.label}
                    </a>
